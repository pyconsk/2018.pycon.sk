...
# .ics related templates
CALENDAR_HEADER = """\
BEGIN:VCALENDAR
PRODID:-//SPy, o.z.//calendar generated by pycon.sk//EN
VERSION:2.0
CALSCALE:GREGORIAN
METHOD:PUBLISH
X-WR-CALNAME:PyCon SK 2018
X-WR-TIMEZONE:Europe/Bratislava
X-WR-CALDESC:Calendar for PyconSK 2018 event
"""
# check if Europe/Bratislava works, use Europe/Vienna if needed
...
def ical_prenormalize(text):
    # if multiline, adds a space at the beginning
...
def generate_event(track, slot):
    ...
    description = flg + talk.get('abstract', '')
    ...
    values = {'start': start, 'end': end, 'exported': exported, 'created': created,
              ...
              'description': norm_description, 'status': status, 'summary': norm_summary,
              'transp': transp, }

    raw_content = EVENT_TEMPLATE.format(**values)
    normalized_content = ''
    for line in raw_content.split('\n'):
        if not len(line):
            continue
        normalized_content += line[:75] + '\n'
        rest = line[75:]
        while rest:
...
@app.route('/<lang_code>/ics.html')
def generate_ics():
    omni_schedule = generate_schedule(API_DATA_TALKS)

    ics_body = CALENDAR_HEADER
